...
```
(npm run sync; cd site; vendor/bin/phpunit tests/unit/Espo/Modules/SincronizacionReferido)
```
...
```
(npm run sync; cd site; vendor/bin/phpunit tests/integration/Espo/Modules/SincronizacionReferido)
```
...
If your extension contains additional PHP packages, you also need to add `site/custom/Espo/Modules/SincronizacionReferido/vendor` to the *scanDirectories* section in *phpstan.neon* config.
...
* `site/custom/`
* `site/client/custom/`
* `site/tests/unit/Espo/Modules/SincronizacionReferido`
* `site/tests/integration/Espo/Modules/SincronizacionReferido`
...
2. Set *bundled* and *jsTranspiled* to true in `src/files/custom/Espo/Modules/SincronizacionReferido/Resources/module.json`.
3. Add `src/files/custom/Espo/Modules/SincronizacionReferido/Resources/metadata/app/client.json`
    ```json
    {
        "scriptList": [
            "__APPEND__",
            "client/custom/modules/sincronizacion-referido/lib/init.js"
        ]
    }
    ```
...
Add the library module path to `src/files/custom/Espo/Modules/SincronizacionReferido/Resources/metadata/app/jsLibs.json`

```json
{
    "some-lib": {
        "path": "client/custom/modules/sincronizacion-referido/lib/some-lib.js"
    }
}
```
